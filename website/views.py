...
def notes():
    if request.method == "POST":
        ...
        if len(title) < 1:
            ...
            new_note = Note(
                data=note, user_id=current_user.id, title=title
...
def orders():
    if request.method == "POST":
        ...
        sum_of_prod = (
            {}
        )  # dict w którym bede trzymać wszystkie produkty ale tylko raz i sumując ile ich łącznie jest
        ...
        return render_template(
            "orders.html", user=current_user, prod=prod, sum_of_prod=sum_of_prod
        )
...
@views.route(
    "/delete-order", methods=["POST"]
)  # tak samo jak z notatkami tylko na produkcie dla jednego zamówienia a nie eby usunąć wszystkie
def delete_order():
...
@views.route(
    "/edit_note/<int:id>", methods=["POST", "GET"]
)  # tu edituje notatki i uywam do tego id w linku
@login_required
def get_note(id):
            ...
            if request.method == "POST":
                ...
                if len(title) < 1:
                    ...
                    new_note = Note(
                        ...
                        id=old_note.id,
                        date_now=old_date,
                    )  # ustalam jakie dane ma mieć nowa notatka, zostawiając stare id
...
def search():
    ...
    if form.validate_on_submit():
        ...
        notes = notes.filter(
            Note.data.like("%" + searched + "%")
        )  # wyszukuje notatke taka która jest podobna
        ...
        if len(searched) < 1:
            ...
            return render_template(
                "search.html", form=form, searched=searched, user=user, notes=notes
            )
